...
- [x] **MayaConnection Forced Shutdown** — Added `force` flag to `MayaConnection.shutdown` and `close_instance` to cleanly forcefully exit Maya process without triggering blocking "Save Changes" prompts when a scene has been modified.
- [x] **Default Camera Exclusion** — Added `CamUtils.DEFAULT_CAMERAS` frozenset as centralized SSoT for Maya default camera names (persp, top, front, side, back, bottom, left, right, alignToPoly). Updated `group_cameras` to use it. Scene exporter `_initialize_objects` now filters out default cameras automatically. Updated `hierarchy_manager` to import from `CamUtils` instead of maintaining a separate constant.