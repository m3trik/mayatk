...
class TestPrimitivesCreation(unittest.TestCase):
    ...
    def test_create_circle_default(self):
        ...
        planar_trims = [
            n for n in history_nodes if pm.nodeType(n) == "planarTrimSurface"
        ]
        nurb_circles = [n for n in history_nodes if pm.nodeType(n) == "makeNurbCircle"]

        self.assertEqual(
            len(planar_trims), 0, "Default circle should not have PlanarTrim history"
        )
        self.assertEqual(
            len(nurb_circles), 0, "Default circle should not have NurbCircle history"
        )

    def test_create_circle_history(self):
        """Test circle creation with history."""
        node_list = Primitives.create_circle(
            name="test_circle_history", history=True, radius=6, numPoints=8
        )
        ...
        self.assertTrue(
            len(planar_trims) > 0, "History circle should have PlanarTrim history"
        )
        self.assertTrue(
            len(nurb_circles) > 0, "History circle should have NurbCircle history"
        )
        ...
        self.assertFalse(
            pm.getAttr(crv_name + ".visibility"), "Source curve should be hidden"
        )

    def test_create_helix(self):
        """Test helix creation."""
        # Using create_default_primitive which likely calls the dictionary lookup
        node = Primitives.create_default_primitive(
            "polygon", "helix", name="test_helix", coils=4
        )
