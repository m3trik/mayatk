...
class _StructuralChecks:
    ...
    def test_all_goto_targets_exist(self):
        missing = [
            (t, ln)
            for t, ln in self.analyzer.gotos
            if t.lower() != ":eof" and t.lstrip(":") not in self.analyzer.labels
        ]
        self.assertFalse(
            missing, f"Unresolved goto in {self.analyzer.path.name}: {missing}"
        )

    def test_all_call_targets_exist(self):
        missing = [
            (t, ln) for t, ln in self.analyzer.calls if t not in self.analyzer.labels
        ]
        self.assertFalse(
            missing, f"Unresolved call :sub in {self.analyzer.path.name}: {missing}"
        )
    ...
    def test_setlocal_has_endlocal(self):
        ...
        self.assertGreaterEqual(
            el, sl, f"{self.analyzer.path.name}: ENDLOCAL {el} < SETLOCAL {sl}"
        )
    ...
    def test_status_markers_escape_the_bang(self):
        ...
        offenders = [
            (i, l.strip()[:80])
            for i, raw in enumerate(self.analyzer.lines, start=1)
            for l in [_strip_comments(raw)]
            if "[!!]" in l
        ]
        self.assertFalse(
            offenders, f"{self.analyzer.path.name}: unescaped `!`: {offenders}"
        )
    ...
    def test_file_is_ascii_only(self):
        ...
        try:
            ...
            self.fail(
                f"{self.analyzer.path.name}: non-ASCII byte at {e.start}: {e.object[e.start : e.end]!r}"
            )
    ...
    def test_powershell_strings_have_no_bare_apostrophe(self):
        ...
        self.assertFalse(
            offenders, f"{self.analyzer.path.name}: unescaped apostrophe: {offenders}"
        )
    ...
    def test_powershell_takes_paths_from_the_environment(self):
        ...
        offenders = [
            ...
            for v in UNSAFE_PS_VARS
            if v in l
        ]
        self.assertFalse(
            offenders,
            f"{self.analyzer.path.name}: inline path in a PS command: {offenders}",
        )
    ...
    def test_powershell_uses_no_profile(self):
        offenders = [
            ...
            for i, raw in enumerate(self.analyzer.lines, start=1)
            for l in [_strip_comments(raw)]
            if re.search(r"\bpowershell\b\s+(?!.*-NoProfile)", l, re.IGNORECASE)
            and "-Command" in l
        ]
        self.assertFalse(
            offenders,
            f"{self.analyzer.path.name}: powershell missing -NoProfile: {offenders}",
        )
...
class TestGenericMenu(_StructuralChecks, unittest.TestCase):
    ...
    def test_required_menu_labels_present(self):
        ...
        self.assertFalse(
            sorted(required - set(self.analyzer.labels)),
    ...
    def test_choice_dispatcher_covers_every_key(self):
        ...
        for idx, start in enumerate(choice_lines):
            ...
            covered = set(levels)
            problems += [
                f"CHOICE@{start} (/C:{keys}) missing ERRORLEVEL {k}"
    ...
    def test_no_in_menu_elevation_relaunch(self):
        ...
        self.assertNotRegex(
            text, r"(?i)-Verb\s+RunAs", "no in-process elevation relaunch"
        )
        self.assertNotIn("Run as Administrator", text)
        self.assertRegex(
            text,
            r'(?im)^fltmc\s*>nul\s*2>&1\s*&&\s*set\s+"mode= \(ADMINISTRATOR\)"',
            "elevation state must be detected without prompting (fltmc)",
        )
        for sec in ("intro", "main"):
            self.assertIn(
                "PACKAGE MANAGER%mode%",
                self.analyzer.section(sec),
    ...
    def test_error_exit_waits_for_the_user(self):
        ...
        self.assertNotRegex(
            block, r"(?i)timeout\s+/t", "validateInterp must not auto-close"
        )
    ...
    def test_result_reports_a_failed_operation(self):
        ...
        self.assertRegex(
            block,
            r'set "op_rc=%ERRORLEVEL%"',
            "the op's exit code must be captured before ECHO clobbers it",
        )
        self.assertIn(
            "firewall", block.lower(), f":result must name the likely cause:\n{block}"
        )
    ...
    def test_prompt_splits_a_list_without_a_pipe(self):
        ...
        self.assertIn(
            'set "module=!module:, = !"', block, "comma+space must always separate"
        )
        self.assertNotIn(
            "|", block, "a pipe re-parses the expanded value; use unpiped tests"
        )
        self.assertIn("delims==", block, "`=` needs for/f, not the replace syntax")
        for ch in ("<", ">", "["):
            self.assertIn(
                f"!module:{ch}=!", block, f"no guard for {ch!r} in a requirement"
            )
        ...
        for idx, (start, name) in enumerate(ordered):
            ...
            for ln in range(end, start, -1):
                ...
                if s:
                    ...
                    break
            if not (
                last.startswith("goto :eof")
                or last.startswith("goto:eof")
                or last.startswith("exit")
                or GOTO_RE.search(last)
            ):
                problems.append(f":{name} ends with {last!r}")
        self.assertFalse(
            problems, "Helper subs must end with goto :eof:\n" + "\n".join(problems)
        )
...
class TestMayapyWrapper(_StructuralChecks, unittest.TestCase):
    ...
    def test_hands_off_to_generic(self):
        ...
        self.assertIn(
            "package-manager.bat",
            text,
            "Wrapper must call the shared package-manager.bat",
        )
        self.assertRegex(
            text, r'call\s+"%generic%"', "Wrapper must `call` the resolved generic"
        )
    ...
    def test_single_install_skips_the_version_prompt(self):
        ...
        self.assertIn(
            "set /a version_count+=1", text, "Wrapper must count detected installs"
        )
        ...
        self.assertNotIn(
            "set /p", branch, f"The single-install branch must not prompt:\n{branch}"
        )
    ...
    def test_failed_bootstrap_holds_the_window_open(self):
        ...
        self.assertNotRegex(
            block, r"(?i)timeout\s+/t", ":fetchFailed must not auto-close"
        )
        self.assertIn(
            "firewall", block.lower(), ":fetchFailed must name the likely cause"
        )
    ...
    def test_every_menu_candidate_is_vetted_before_it_is_called(self):
        ...
        self.assertIn(
            "checkGeneric", self.analyzer.labels, "Wrapper missing :checkGeneric"
        )
        block = self.analyzer.section("checkGeneric")
        self.assertRegex(
            block,
            r"findstr\s+/b\s+/c:\":main\"",
            f"the menu's signature must be checked, not just existence:\n{block}",
        )
        self.assertRegex(
            block, r'set "generic="', "a rejected candidate must be cleared"
        )
        handoff = self.analyzer.section("handoff")
        self.assertEqual(
            handoff.count("call :checkGeneric"),
            2,
            f"both on-disk candidates must be vetted:\n{handoff}",
        )
        self.assertNotRegex(
            handoff,
            r'if exist "%generic%" goto runGeneric',
    ...
    def test_bootstrap_fetches_through_the_resolved_interpreter(self):
        ...
        block = self.analyzer.section("fetchShared") + self.analyzer.section(
            "fetchWheel"
        )
        self.assertIn(
            '"%mayapy%" -m pip download',
            block,
        ...
        for flag in ("--no-deps", "--only-binary=:all:", "--retries", "--timeout"):
            self.assertIn(
                flag, block, f"{flag} missing from the bootstrap download:\n{block}"
            )
    ...
    def test_failed_download_leaves_nothing_behind(self):
        ...
        self.assertIn(
            "call :checkGeneric", block, "the download is vetted like any candidate"
        )
        self.assertRegex(
            block,
            r"if defined generic goto :eof\ndel /f /q",
            f"an unusable download must be deleted:\n{block}",
        )
        self.assertRegex(
            block,
            r'if not exist "%fetch_target%" goto :eof',
            "a failed stage must keep its own name for :fetchFailed to report",
        )
        self.assertEqual(
            block.count('rd /s /q "%fetch_dir%"'),
            2,
    ...
    def test_unentered_version_is_a_dead_end(self):
        ...
        self.assertIn(
            "pause", block.lower(), f":noVersion must hold the window:\n{block}"
        )
    ...
    def test_shared_menu_mirrored_next_to_wrapper(self):
        ...
        self.assertTrue(
            MIRROR_PATH.is_file(),
            f"Shared menu not mirrored next to wrapper: {MIRROR_PATH}",
        )
...
class TestPromptSeparators(unittest.TestCase):
    ...
    def setUp(self):
        ...
        lines = (
            GENERIC_PATH.read_bytes().decode("utf-8").replace("\r\n", "\n").split("\n")
        )
        start = lines.index(":promptModule")
        end = next(
            i for i in range(start + 1, len(lines)) if LABEL_DEF_RE.match(lines[i])
        )
...
class TestBlockedBootstrap(unittest.TestCase):
    ...
    def test_blocked_fetch_explains_and_waits(self):
        ...
        self.assertFalse(
            self.scratch.exists(), f"Wheel scratch dir survived: {self.scratch}"
        )
    ...
    def test_stale_stub_is_rejected_rather_than_called(self):
        ...
        self.assertIn(
            "firewall", out.lower(), f"No explanation reached the user:\n{out}"
        )
    ...
    def test_an_apostrophe_in_the_path_does_not_eat_the_message(self):
        ...
        self.assertNotIn(
            ...
            out + err,
            f"PowerShell parser error:\n{out}\n{err}",
        )
        self.assertIn(
            "Save this file by hand", out, f"The remedies block was eaten:\n{out}"
        )
    ...
    def test_no_install_and_no_input_terminates(self):
        ...
        self.assertEqual(proc.returncode, 1, f"Expected a failure exit.\n{out}")
        self.assertIn(
            "nothing to do", out.lower(), f"No explanation reached the user:\n{out}"
        )
        self.assertIn("Press any key", out, f"Window would close unattended:\n{out}")
