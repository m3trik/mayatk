...
class TestInterpolatedSegments(MayaTkTestCase if pm else unittest.TestCase):
    """Tests for interpolated-motion segments (no physical keys in range)."""
    ...
    def test_interpolated_segment_created(self):
        """A range with no keys but non-flat interpolation produces a segment."""
        cube = pm.polyCube(name="interp_cube")[0]
        pm.setKeyframe(cube, t=0, v=0, at="tx")
        pm.setKeyframe(cube, t=100, v=100, at="tx")

        # Shot range 30-60 has no physical keys but value is changing
        ...
        self.assertTrue(segs[0].get("interpolated", False))
        self.assertAlmostEqual(segs[0]["start"], 30.0)
        self.assertAlmostEqual(segs[0]["end"], 60.0)
        self.assertEqual(segs[0]["keyframes"], [])

    def test_keyed_range_not_marked_interpolated(self):
        """A range with physical keys is not marked as interpolated."""
        cube = pm.polyCube(name="keyed_cube")[0]
        pm.setKeyframe(cube, t=0, v=0, at="tx")
        pm.setKeyframe(cube, t=50, v=50, at="tx")
        pm.setKeyframe(cube, t=100, v=100, at="tx")
        ...
        self.assertEqual(len(segs), 1)
        self.assertFalse(segs[0].get("interpolated", False))
        self.assertIn(50.0, segs[0]["keyframes"])

    def test_flat_interpolation_produces_no_segment(self):
        """A range where all curves are flat produces no segment."""
        cube = pm.polyCube(name="flat_cube")[0]
        pm.setKeyframe(cube, t=0, v=5, at="tx")
        pm.setKeyframe(cube, t=100, v=5, at="tx")
    ...
    def test_interpolated_with_ignore_holds_false(self):
        """Interpolated path does not crash when ignore_holds=False."""
        cube = pm.polyCube(name="interp_holds_cube")[0]
        pm.setKeyframe(cube, t=0, v=0, at="tx")
        pm.setKeyframe(cube, t=100, v=100, at="tx")
