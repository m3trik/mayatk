...
# coding=utf-8
"""
Test Suite for AttributeManager
...
from mayatk.node_utils.attributes.attribute_manager import AttributeManager


class TestFormatValue(MayaTkTestCase):
    """Tests for AttributeManager.format_value."""

    def test_none_returns_empty(self):
        self.assertEqual(AttributeManager.format_value(None), "")

    def test_star_passthrough(self):
        self.assertEqual(AttributeManager.format_value("*"), "*")

    def test_float_precision(self):
        self.assertEqual(AttributeManager.format_value(1.23456789), "1.2346")

    def test_int_passthrough(self):
        self.assertEqual(AttributeManager.format_value(42), "42")

    def test_tuple_formatted(self):
        result = AttributeManager.format_value((1.0, 2.0, 3.0))
        self.assertEqual(result, "(1.0000, 2.0000, 3.0000)")

    def test_string_passthrough(self):
        self.assertEqual(AttributeManager.format_value("hello"), "hello")

    def test_bool_passthrough(self):
        self.assertEqual(AttributeManager.format_value(True), "True")


class TestParseValue(MayaTkTestCase):
    """Tests for AttributeManager.parse_value."""

    def test_double(self):
        self.assertAlmostEqual(
            AttributeManager.parse_value("3.14", "double"), 3.14
        )

    def test_float_type(self):
        self.assertAlmostEqual(
            AttributeManager.parse_value("2.5", "float"), 2.5
        )

    def test_long_truncates(self):
        self.assertEqual(AttributeManager.parse_value("7.9", "long"), 7)

    def test_bool_true_variants(self):
        for text in ("1", "true", "True", "yes", "on"):
            self.assertTrue(
                AttributeManager.parse_value(text, "bool"),
                f"Expected True for '{text}'",
            )

    def test_bool_false(self):
        self.assertFalse(AttributeManager.parse_value("0", "bool"))

    def test_string_passthrough(self):
        self.assertEqual(AttributeManager.parse_value("abc", "string"), "abc")

    def test_enum_int(self):
        self.assertEqual(AttributeManager.parse_value("2", "enum"), 2)

    def test_enum_label_fallback(self):
        self.assertEqual(AttributeManager.parse_value("Red", "enum"), "Red")

    def test_unsupported_returns_none(self):
        self.assertIsNone(AttributeManager.parse_value("x", "compound"))


class TestSortChannelBox(MayaTkTestCase):
    """Tests for AttributeManager._sort_channel_box."""

    def test_canonical_order(self):
        attrs = ["scaleX", "translateX", "rotateX", "visibility"]
        result = AttributeManager._sort_channel_box(attrs)
    ...
    def test_empty_list(self):
        self.assertEqual(AttributeManager._sort_channel_box([]), [])


class TestGetFilterKwargs(MayaTkTestCase):
    """Tests for AttributeManager.get_filter_kwargs."""

    def test_custom_filter(self):
        kw = AttributeManager.get_filter_kwargs("Custom")
        self.assertIn("userDefined", kw)

    def test_keyable_filter(self):
        kw = AttributeManager.get_filter_kwargs("Keyable")
        self.assertIn("keyable", kw)

    def test_all_filter(self):
        kw = AttributeManager.get_filter_kwargs("All")
        self.assertEqual(kw, {})

    def test_unknown_filter_defaults_custom(self):
        kw = AttributeManager.get_filter_kwargs("NonExistent")
        self.assertIn("userDefined", kw)
...
class TestToggleLock(MayaTkTestCase):
    ...
    def test_toggle_lock_on_and_off(self):
        ...
        AttributeManager.toggle_lock([self.cube], "translateX")
        self.assertTrue(cmds.getAttr(f"{self.cube}.tx", lock=True))
        AttributeManager.toggle_lock([self.cube], "translateX")
        self.assertFalse(cmds.getAttr(f"{self.cube}.tx", lock=True))

    def test_set_lock_explicit(self):
        AttributeManager.set_lock([self.cube], ["translateX"], True)
        self.assertTrue(cmds.getAttr(f"{self.cube}.tx", lock=True))
        AttributeManager.set_lock([self.cube], ["translateX"], False)
        self.assertFalse(cmds.getAttr(f"{self.cube}.tx", lock=True))
...
class TestBreakConnections(MayaTkTestCase):
    ...
    def test_break_connected(self):
        ...
        result = AttributeManager.break_connections([self.cube], "translateX")
    ...
    def test_break_unconnected_returns_false(self):
        result = AttributeManager.break_connections([self.cube], "translateY")
...
class TestBuildTableData(MayaTkTestCase):
    ...
    def test_custom_filter_returns_empty_for_cube(self):
        ...
        rows, states = AttributeManager.build_table_data(
    ...
    def test_multi_node_mixed_value(self):
        ...
        rows, _ = AttributeManager.build_table_data(
...
class TestResetToDefault(MayaTkTestCase):
    ...
    def test_reset_translation(self):
        ...
        AttributeManager.reset_to_default([self.cube], ["translateX"])
...
class TestCreateAttribute(MayaTkTestCase):
    ...
    def test_create_float_attr(self):
        AttributeManager.create_attribute(
    ...
    def test_create_bool_attr(self):
        AttributeManager.create_attribute([self.cube], "myBool", "bool")
        self.assertTrue(cmds.attributeQuery("myBool", node=self.cube, exists=True))

    def test_create_double3_attr(self):
        AttributeManager.create_attribute([self.cube], "myVec", "double3")
    ...
    def test_duplicate_attr_warns_not_errors(self):
        ...
        AttributeManager.create_attribute([self.cube], "myFloat", "float")
...
class TestRenameAttribute(MayaTkTestCase):
    ...
    def test_rename_success(self):
        result = AttributeManager.rename_attribute(
    ...
    def test_rename_empty_returns_false(self):
        result = AttributeManager.rename_attribute([self.cube], "oldName", "")
        self.assertFalse(result)
...
class TestDeleteAttributes(MayaTkTestCase):
    ...
    def test_delete_custom_attr(self):
        ...
        AttributeManager.delete_attributes([self.cube], ["toDelete"])
...
class TestGetShapeNodes(MayaTkTestCase):
    ...
    def test_returns_shape(self):
        shapes = AttributeManager.get_shape_nodes([self.cube])
    ...
    def test_empty_for_group(self):
        ...
        shapes = AttributeManager.get_shape_nodes([grp])
...
class TestGetHistoryNodes(MayaTkTestCase):
    ...
    def test_returns_polycube_node(self):
        history = AttributeManager.get_history_nodes([self.cube])
...
class TestSetBreakdownKey(MayaTkTestCase):
    ...
    def test_creates_breakdown_key(self):
        ...
        AttributeManager.set_breakdown_key([self.cube], ["translateX"])
...
class TestMuteUnmute(MayaTkTestCase):
    ...
    def test_mute_and_unmute(self):
        ...
        AttributeManager.unmute_attrs([self.cube], ["translateX"])
...
class TestToggleKeyable(MayaTkTestCase):
    ...
    def test_toggle_off_and_on(self):
        ...
        AttributeManager.toggle_keyable([self.cube], ["translateX"])
...
class TestSelectConnections(MayaTkTestCase):
    ...
    def test_selects_upstream(self):
        result = AttributeManager.select_connections(
