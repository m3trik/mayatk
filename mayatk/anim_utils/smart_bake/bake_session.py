...
class BakeSessionStore(_BakeSessionStoreInternal):
    ...
    def stash_curve(curve: str) -> dict:
        ...
        record["stash"] = BakeSessionStore.node_ref(dup)
        return record
    ...
    def unstash_curve(
        ...
        for src_ref, curve_attr in record.get("inputs", []):
            src = BakeSessionStore.resolve_plug(src_ref)
            if not src:
                warnings.append(
        ...
        for curve_attr, dst_ref in record.get("outputs", []):
            dst = BakeSessionStore.resolve_plug(dst_ref)
            if not dst:
                warnings.append(
            ...
            try:
                ...
                warnings.append(
                    f"Could not reconnect '{stash}.{curve_attr}' -> '{dst}': {e}"
                )
    ...
    def snapshot_connections(plug: str) -> List[List[dict]]:
        ...
        for p in plugs_to_check:
            ...
            for i in range(0, len(conns), 2):
                dst = conns[i]
                src = _BakeSessionStoreInternal._trace_out_of_conversion(conns[i + 1])
    ...
    def restore_session(session: dict) -> "RestoreResult":
        ...
        for entry in session.get("baked_plugs", []):
            node = BakeSessionStore.resolve_ref(entry.get("ref"))
            if not node:
                result.warnings.append(
        ...
        for record in session.get("stashed_curves", []):
            restored = BakeSessionStore.unstash_curve(record, warnings=result.warnings)
            if restored:
        ...
        for entry in session.get("visibility", []):
            ...
            _BakeSessionStoreInternal._delete_plug_curves(vis_plug)
        ...
        for entry in session.get("ik_handles", []):
            handle = BakeSessionStore.resolve_ref(entry.get("ref"))
            if not handle:
                result.warnings.append(
            ...
            try:
                if not entry.get("had_incoming", False):
                    _BakeSessionStoreInternal._delete_plug_curves(f"{handle}.ikBlend")
