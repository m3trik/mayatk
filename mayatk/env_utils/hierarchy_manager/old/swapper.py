...
class ObjectMatcher(ptk.LoggingMixin):
    """Handles object matching logic including fuzzy matching and container searches."""

    def __init__(self, import_manager: TempImport, fuzzy_matching: bool = True):
        super().__init__()
        self.import_manager = import_manager
        self.fuzzy_matching = fuzzy_matching

    def find_matching_objects(
        self, target_objects: List[str], imported_transforms: List
    ) -> Tuple[List, Dict]:
        ...
        for target_name in target_objects:
            ...
            self._log_debug_info(target_name, imported_transforms)

            # Try fuzzy matching if enabled
            if self.fuzzy_matching:
                match_result = self._find_fuzzy_match(target_name, imported_transforms)
                if match_result:
    ...
    def _log_debug_info(self, target_name: str, imported_transforms: List) -> None:
        ...
        available_containing_target = [
            ...
            for node in imported_transforms
            if target_name in self.import_manager._clean_namespace_name(node.nodeName())
        ]
        if available_containing_target:
            ...
            self.logger.debug(f"No objects found containing '{target_name}'")
    ...
    def _find_fuzzy_match(
        self, target_name: str, imported_transforms: List
        ...
        try:
            ...
            self.logger.debug("Using single best match (return_all not supported)")
        ...
        for matched_name, score in potential_matches:
            ...
            # Enhanced fuzzy matching: Search inside containers for better matches
            better_match_result = self._search_container_for_better_match(
                matching_node, target_name, matched_name, score
            )

            if better_match_result:
                matching_node, matched_name, score, should_use_match = (
                    better_match_result
                )
    ...
    def _search_container_for_better_match(
        self, matching_node: Any, target_name: str, matched_name: str, score: float
        ...
        if hasattr(matching_node, "getChildren"):
            try:
                ...
                def find_better_match_recursive(
                    ...
                    for child in children:
                        ...
                        if is_exact_match:
                            ...
                            len(child_clean_name) > len(matched_name)
                        ...
                        if child.getChildren():
                            recursive_result, recursive_has_shapes = (
                                find_better_match_recursive(
                                    child, target_name, depth + 1, max_depth
                                )
                            )
                            if recursive_result:
                ...
                better_child, child_has_shapes = find_better_match_recursive(
                ...
                self.logger.debug(f"Could not search inside container: {search_error}")
...
class SceneManager(ptk.LoggingMixin):
    """Handles scene operations including import/export and scene switching."""

    def __init__(self, import_manager: TempImport, dry_run: bool = True):
        super().__init__()
        self.import_manager = import_manager
        self.dry_run = dry_run
    ...
    def import_source_objects(self, source_file: Path) -> Optional[Dict]:
        """Import objects from source scene and return import info."""
        try:
...
class ObjectProcessor(ptk.LoggingMixin):
    ...
    def __init__(self, import_manager: TempImport):
        super().__init__()
        self.import_manager = import_manager
    ...
    def _collect_parent_info(
        ...
        for obj in found_objects:
            try:
                ...
                # Handle existing object replacement
                parent_info.update(
        ...
        if pm.objExists(clean_name):
            ...
            try:
                ...
                try:
                    ...
                    try:
                        obj_path = (
                            obj.fullPath() if hasattr(obj, "fullPath") else str(obj)
                        )
                    except:
                        obj_path = str(obj)
                    ...
                    elif obj_path != existing_path and obj_path and existing_path:
                        objects_are_different = True
                        self.logger.debug(
                    ...
                    self.logger.debug(f"Could not compare objects safely: {comp_error}")
            ...
            self.logger.debug(f"No existing object named {clean_name} to replace")
        ...
        try:
            ...
            if ":" in original_name:
                ...
                try:
                    ...
                    self.logger.debug(f"Could not parent to world: {parent_error}")
    ...
    def _cleanup_unused_objects(
        ...
        for obj in imported_objects:
            if (
                obj not in processed_objects  # Not one of the objects we processed
                and obj not in objects_to_preserve  # Not in hierarchy to preserve
                and pm.objExists(obj)  # Still exists
            ):
...
class DryRunAnalyzer(ptk.LoggingMixin):
    """Handles dry-run analysis using enhanced fuzzy matching."""

    def __init__(self, import_manager: TempImport, object_matcher: ObjectMatcher):
        super().__init__()
        self.import_manager = import_manager
        self.object_matcher = object_matcher

    def analyze_import(self, source_file: Path, target_objects: List[str]) -> bool:
        ...
        try:
            ...
            found_objects, fuzzy_match_map = self._analyze_matches(
                target_objects, imported_transforms
            )

            # Report results
            if found_objects:
                self._log_analysis_results(found_objects, fuzzy_match_map)
            else:
            ...
            if imported_transforms:
                self._cleanup_analysis_namespace(imported_transforms)

    def _analyze_matches(
        self, target_objects: List[str], imported_transforms: List
    ) -> Tuple[List, Dict]:
        """Analyze matches using the same logic as real import."""
        found_objects = []
        fuzzy_match_map = {}
        ...
        for target_name in target_objects:
            ...
            # Log debug info
            self._log_dry_run_debug_info(target_name, imported_transforms)

            # Try fuzzy matching if enabled
            if self.object_matcher.fuzzy_matching:
                match_result = self._find_dry_run_fuzzy_match(
                    target_name, imported_transforms
                )
                if match_result:
                    matching_node, fuzzy_target_name = match_result
                    found_objects.append(matching_node)
    ...
    def _log_dry_run_debug_info(
        self, target_name: str, imported_transforms: List
    ) -> None:
        """Log debug information for dry run analysis."""
        # Debug: Show available objects that contain the target name
        available_containing_target = [
            self.import_manager._clean_namespace_name(node.nodeName())
            for node in imported_transforms
        ...
        if available_containing_target:
            ...
            self.logger.debug(f"[DRY-RUN] No objects found containing '{target_name}'")
        ...
        self.logger.debug(f"[DRY-RUN] Sample available objects: {sample_objects}")

    def _find_dry_run_fuzzy_match(
        self, target_name: str, imported_transforms: List
    ) -> Optional[Tuple[Any, str]]:
        """Find fuzzy match for dry run analysis."""
        # Standard behavior: Skip fuzzy matching when target already exists in current scene
        if pm.objExists(target_name):
            self.logger.warning(
        ...
        for matched_name, score in potential_matches:
            ...
            better_match_result = self._dry_run_container_search(
                matching_node, target_name, matched_name, score
            )
    ...
    def _dry_run_container_search(
        self, matching_node: Any, target_name: str, matched_name: str, score: float
    ) -> Optional[Tuple[Any, str, float, bool]]:
        """Search inside containers for better matches in dry run mode."""
        better_match_found = False
    ...
    def _log_analysis_results(self, found_objects: List, fuzzy_match_map: Dict) -> None:
        ...
        for obj in found_objects:
            ...
            actual_name = self.import_manager._clean_namespace_name(obj.nodeName())
            target_name = fuzzy_match_map.get(obj, actual_name)
            self.logger.notice(
                f"[DRY-RUN] Would pull: {actual_name} (for target: {target_name})"
            )

    def _cleanup_analysis_namespace(self, imported_transforms: List) -> None:
        """Clean up analysis namespace and imported objects."""
        try:
            ...
            if pm.namespace(exists=namespace):
                try:
                    ...
                    self.logger.debug(f"[DRY-RUN] Removed namespace: {namespace}")
                except Exception as ns_error:
                    self.logger.debug(f"[DRY-RUN] Error removing namespace: {ns_error}")
                    # Try alternative cleanup approach
                    self._alternative_namespace_cleanup(namespace)

            # Step 3: Verify cleanup was successful
            self._verify_cleanup(namespace)

        except Exception as cleanup_error:
            self.logger.warning(f"[DRY-RUN] Cleanup failed: {cleanup_error}")
            # As a last resort, try to list what's still there
            self._log_remaining_objects()

    def _alternative_namespace_cleanup(self, namespace: str) -> None:
        """Alternative namespace cleanup approach."""
        try:
            ...
            if pm.namespace(exists=namespace):
                ...
                self.logger.debug(f"[DRY-RUN] Removed empty namespace: {namespace}")
        except Exception as manual_error:
            self.logger.warning(f"[DRY-RUN] Manual cleanup failed: {manual_error}")

    def _verify_cleanup(self, namespace: str) -> None:
        """Verify that cleanup was successful."""
        remaining_objects = (
            pm.ls(f"{namespace}:*") if pm.namespace(exists=namespace) else []
        )
        if remaining_objects:
            self.logger.warning(
            ...
            try:
                ...
                if pm.namespace(exists=namespace):
                    pm.namespace(removeNamespace=namespace)
                self.logger.debug("[DRY-RUN] Final aggressive cleanup completed")
            except:
                self.logger.error(
                    f"[DRY-RUN] FAILED TO CLEAN UP: {len(remaining_objects)} objects remain in {namespace}"
                )
        else:
            self.logger.debug(f"[DRY-RUN] Cleanup successful - no objects remain")

    def _log_remaining_objects(self) -> None:
        """Log any remaining objects after cleanup attempts."""
        try:
            namespace = self.import_manager.DRY_RUN_NAMESPACE
            remaining = (
                pm.ls(f"{namespace}:*") if pm.namespace(exists=namespace) else []
...
class ObjectSwapper(ptk.LoggingMixin):
    ...
    def __init__(
        ...
        self.validator = ValidationManager(dry_run=dry_run)
        self.object_matcher = ObjectMatcher(
            self.import_manager, fuzzy_matching=fuzzy_matching
        )
        self.scene_manager = SceneManager(self.import_manager, dry_run=dry_run)
        self.object_processor = ObjectProcessor(self.import_manager)
        self.dry_run_analyzer = DryRunAnalyzer(self.import_manager, self.object_matcher)
    ...
    def push_objects_to_scene(
        ...
        if not self.validator.validate_inputs(object_names, target_scene_file, "Push"):
            return False
        ...
        try:
            ...
            current_scene = pm.sceneName()
            success = self.scene_manager.import_to_target_scene(
    ...
    def pull_objects_from_scene(
        ...
        if not self.validator.validate_inputs(object_names, source_scene_file, "Pull"):
            return False
    ...
    def _import_source_objects(
        ...
        try:
            ...
            if self.dry_run:
                ...
                return self.dry_run_analyzer.analyze_import(source_file, target_objects)

            # Import objects and get import information
            import_info = self.scene_manager.import_source_objects(source_file)
            if not import_info:
                return False
            ...
            try:
                ...
                found_objects, fuzzy_match_map = (
                    self.object_matcher.find_matching_objects(
                        target_objects, imported_transforms
                    )
                )
                ...
                success = self.object_processor.process_found_objects_with_cleanup(
                ...
                if "imported_transforms" in locals() and "found_objects" in locals():
                    self._cleanup_on_error(imported_transforms, found_objects)
...
if __name__ == "__main__":
    ...
    pull_objects_from_scene(objs, path, fuzzy_matching=1, dry_run=1)
