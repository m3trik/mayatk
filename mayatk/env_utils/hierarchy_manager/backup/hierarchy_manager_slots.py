...
class HierarchyManagerSlots(ptk.LoggingMixin):
    ...
    def tb001_init(self, widget):
        ...
        # Add diff display options
        widget.menu.add(
            "QCheckBox",
            setText="Auto-Select Differences",
            setObjectName="chk_auto_select",
            setChecked=True,
            setToolTip="Automatically select differences in trees.",
        )
        widget.menu.add(
            "QCheckBox",
            setText="Select Root Only",
            setObjectName="chk_select_root_only",
            setChecked=False,
            setToolTip="Optional: Only select root differences (ignore children when parent is selected). Useful for bulk operations on entire hierarchies.",
        )
        widget.menu.add(
            "QCheckBox",
            setText="Select Leaves Only",
            setObjectName="chk_select_leaves_only",
            setChecked=False,
            setToolTip="Optional: Only select leaf differences (deepest level objects). Useful for more targeted pulling when working with complex hierarchies.",
        )
        widget.menu.add(
            "QCheckBox",
    ...
    def tb001(self, state=None):
        ...
        auto_select = True
        select_root_only = False  # Default: select all differences
        select_leaves_only = False  # Default: select all differences (not just leaves)
        expand_diff = True
        ...
        if hasattr(self.ui, "tb001") and hasattr(self.ui.tb001, "menu"):
            ...
            if hasattr(self.ui.tb001.menu, "chk_auto_select"):
                auto_select = self.ui.tb001.menu.chk_auto_select.isChecked()
            if hasattr(self.ui.tb001.menu, "chk_select_root_only"):
                select_root_only = self.ui.tb001.menu.chk_select_root_only.isChecked()
            if hasattr(self.ui.tb001.menu, "chk_select_leaves_only"):
                select_leaves_only = (
                    self.ui.tb001.menu.chk_select_leaves_only.isChecked()
                )
        ...
        if auto_select:
            ...
            if select_root_only and missing_list:
                ...
                if len(missing_list) != original_count:
                    self.logger.debug(
                        f"Condensed missing paths from {original_count} to {len(missing_list)} root paths"
                    )
            ...
            if select_root_only and extra_list:
                ...
                if len(extra_list) != original_extra:
                    self.logger.debug(
                        f"Condensed extra paths from {original_extra} to {len(extra_list)} root paths"
                    )
