...
Auto-pairs meshes by '_high' / '_low' suffix (own name or any ancestor
group). Roundtrip mode runs Toolbag headless and writes one PSD per
enabled map (Normal / AO / Curvature / Thickness / Position / MatID)
into the Output Dir."""
...
def _output_dir():
    """Where baked maps go -- driven by the bridge's required Output Dir."""
    return OUTPUT_DIR
...
def main():
    ...
    _set("outputWidth", BAKE_SIZE)
    _set("outputHeight", BAKE_SIZE)
