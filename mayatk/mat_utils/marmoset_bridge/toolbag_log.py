...
class ToolbagLog:
    ...
    def resolve_toolbag_log_path(toolbag_exe: Optional[str]) -> Optional[str]:
        ...
        if isinstance(toolbag_exe, str) and toolbag_exe:
            ...
            if m:
                ...
                return str(
                    local_app_path / f"Marmoset Toolbag {m.group(1)}" / "log.txt"
                )
    ...
    def dispatch_log_lines(lines, logger) -> None:
        ...
        """
        for raw in lines:
            classified = ToolbagLog.classify_log_line(raw)
            ...
            try:
                ...
                with open(log_path, "r", encoding="utf-8", errors="replace") as fh:
                    ...
                    if tail:
                        buffered += tail
                    if buffered:
                        ToolbagLog.dispatch_log_lines(buffered.split("\n"), logger)
            except Exception:
