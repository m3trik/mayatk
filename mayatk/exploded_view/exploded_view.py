...
#from this package:
from mayatk._xform import Xform 
...
class ExplodedView:
	...
	def calculate_repulsive_force(centroid1, size1, centroid2, size2, scale=0.05):
		...
		force_vector = om.MVector(centroid1[0] - centroid2[0],
	...
	@classmethod
	def arrange_objects(cls, nodes, convergence_threshold=1e-4, max_iterations=1000, max_movement=1.0):
		...
		while not converged and iteration_count < max_iterations:
			...
			for idx1, node1 in enumerate(nodes):
				...
				for idx2, node2 in enumerate(nodes):
					if node1 != node2:
						repulsive_force = cls.calculate_repulsive_force(node_data[idx1][0],
																	node_data[idx1][1],
																	node_data[idx2][0],
																	node_data[idx2][1])
						total_force += repulsive_force

				movement_vector = om.MVector(total_force.x, total_force.y, total_force.z)

				if movement_vector.length() > max_movement:
					movement_vector.normalize()
					movement_vector *= max_movement

				pm.move(movement_vector.x, movement_vector.y, movement_vector.z, node1, relative=True)
				total_system_force += movement_vector

				# Update centroid
				new_centroid = [node_data[idx1][0][i] + movement_vector[i] for i in range(3)]
...
def main(frameless_window=False):
	"""Launch the UI"""
	...
	sb = Switchboard(parent, ui_location='exploded_view.ui', slots_location=ExplodedViewSlots)
	if frameless_window:
