...
class Xform(object):
    ...
    @coretk.Core.undo
    def dropToGrid(
        objects, align="Mid", origin=False, centerPivot=False, freezeTransforms=False
    ):
    ...
    @classmethod
    def sortByBoundingBoxValue(
        cls, objects, value="volume", descending=True, returnWithValue=False
    ):
    ...
    def isOverlapping(objA, objB, tolerance=0.001):
        ...
        closestVerts = cmpttk.Cmpt.getClosestVerts(
    ...
    def getVertPositions(objects, worldSpace=True):
        ...
        result = []
        for mesh in coretk.Core.mfnMeshGenerator(objects):
    ...
    def alignVertices(mode, average=False, edgeloop=False):
        ...
        if len(selection) < 2:
            if len(selection) == 0:
                coretk.Core.viewportMessage("No vertices selected")
            coretk.Core.viewportMessage("Selection must contain at least two vertices")
