...
class Script:
    ...
    @staticmethod
    def convertMelToPy(
        mel, excludeFromInput=[], excludeFromOutput=["from pymel.all import *", "s pm"]
    ):
    ...
    def commandHelp(command):  # mel command help
        ...
        outputscrollField(
            modtext, "command help", 1.0, 1.0
        )  # text, window_title, width, height
    ...
    def queryRuntime(command):  # query runtime command info
        ...
        outputscrollField(
            output_text, "runTimeCommand", 1.0, 1.0
        )  # text, window_title, width, height
    ...
    def searchPymel(keyword):  # search online pymel documentation
        url = "{}{}{}".format(
            "http://download.autodesk.com/global/docs/maya2014/zh_cn/PyMel/search.html?q=",
            keyword,
    ...
    def listUiObjects():  # lsUI returns the names of UI objects
        windows = "{}\n{}\n{}\n".format(
            "Windows", "Windows created using ELF UI commands:", pm.lsUI(windows=True)
        )
        panels = "{}\n{}\n{}\n".format(
            "Panels", "All currently existing panels:", pm.lsUI(panels=True)
        )
        editors = "{}\n{}\n{}\n".format(
            "Editors", "All currently existing editors:", pm.lsUI(editors=True)
        )
        controls = "{}\n{}\n{}\n".format(
            "Controls",
            "Controls created using ELF UI commands: [e.g. buttons, checkboxes, etc]",
            pm.lsUI(controls=True),
        )
        control_layouts = "{}\n{}\n{}\n".format(
            "Control Layouts",
            "Control layouts created using ELF UI commands: [e.g. formLayouts, paneLayouts, etc.]",
            pm.lsUI(controlLayouts=True),
        )
        menus = "{}\n{}\n{}\n".format(
            "Menus", "Menus created using ELF UI commands:", pm.lsUI(menus=True)
        )
        menu_items = "{}\n{}\n{}\n".format(
            "Menu Items",
            "Menu items created using ELF UI commands:",
            pm.lsUI(menuItems=True),
        )
        contexts = "{}\n{}\n{}\n".format(
            "Tool Contexts",
            "Tool contexts created using ELF UI commands:",
            pm.lsUI(contexts=True),
        )
        output_text = "{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}".format(
            windows,
            panels,
