...
class _HotkeyCollisionsInternal(object):
    """Internal helpers for HotkeyCollisions."""
    ...
    def _find_bound_command(parsed: dict) -> str:
        ...
        try:
            name_command = (
                cmds.hotkey(
                    ...
                    query=True,
                    name=True,
                    **_HotkeyCollisionsInternal._hotkey_mod_kwargs(parsed),
                )
                or ""
            )
        except Exception:
        ...
        if not name_command or name_command.strip().lower() == "none":
            return ""
        return (
            _HotkeyCollisionsInternal._runtime_command_for(name_command) or name_command
        )
    ...
    def _is_hotkey_set_editable() -> bool:
        ...
        """
        current = _HotkeyCollisionsInternal._current_hotkey_set()
...
class HotkeyCollisions(_HotkeyCollisionsInternal):
    """HotkeyCollisions — module namespace."""
    ...
    def keystring_to_token(ks: list) -> str:
        ...
        if not ks or not ks[0] or str(ks[0]).strip().lower() == "none":
            return ""
        key = ks[0]
        ctrl = _HotkeyCollisionsInternal._ks_modifier(ks, "ctrl")
        alt = _HotkeyCollisionsInternal._ks_modifier(ks, "alt")
        shift = _HotkeyCollisionsInternal._ks_modifier(ks, "shift")
    ...
    def live_hotkey_map() -> dict:
        ...
        for i in range(1, count + 1):
            ...
            if not cmd or not ks:
                continue
            token = HotkeyCollisions.keystring_to_token(ks)
            if token:
                result[cmd] = token
        return result
    ...
    def ensure_editable_hotkey_set(name: str = MACRO_HOTKEY_SET) -> str:
        ...
        """
        if _HotkeyCollisionsInternal._is_hotkey_set_editable():
            return _HotkeyCollisionsInternal._current_hotkey_set()
        if cmds.hotkeySet(name, query=True, exists=True):
            cmds.hotkeySet(name, edit=True, current=True)
        else:
            cmds.hotkeySet(
                name,
                source=_HotkeyCollisionsInternal._current_hotkey_set()
                or "Maya_Default",
                current=True,
            )
        return name
    ...
    def maya_collision_checker(sequence, scope, ui_name, method_name, ignore=None):
        ...
        parsed = HotkeyCollisions.parse_qt_sequence(sequence)
        if parsed is None:
            return conflicts

        bound = _HotkeyCollisionsInternal._find_bound_command(parsed)
        if not bound or (ignore is not None and ignore(bound)):
            return conflicts

        set_name = _HotkeyCollisionsInternal._current_hotkey_set()
        editable = _HotkeyCollisionsInternal._is_hotkey_set_editable()
