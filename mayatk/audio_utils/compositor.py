...
class _CompositorInternal(object):
    """Internal helpers for Compositor."""
    ...
    def _all_managed_nodes() -> List[str]:
        if cmds is None:
            return []
        return [n for n in (cmds.ls(type="audio") or []) if Compositor.is_managed_dg(n)]
...
class Compositor(_CompositorInternal):
    ...
    def find_dg_node_for_track(track_id: str) -> Optional[str]:
        ...
        for node in _CompositorInternal._all_managed_nodes():
            if _CompositorInternal._marker_value(node) == track_id:
                return node
        return None
    ...
    def sync(
        ...
        managed_by_tid: dict = {}
        for node in _CompositorInternal._all_managed_nodes():
            tid = _CompositorInternal._marker_value(node)
            if tid:
        ...
        for tid in sorted(scope):
            if tid not in all_track_ids:
                ...
                for node in managed_by_tid.get(tid, []):
                    _CompositorInternal._delete_safely(node)
                    deleted.append(node)
                continue
            ...
            if not existing:
                node = _nodes.Nodes.create_dg(
                    path, name=tid, offset=offset, track_id=tid
                )
                if node:
                    created.append(node)
                continue
            ...
            if current_path != playable or abs(current_offset - offset) > 1e-6:
                _nodes.Nodes.configure_dg(primary, playable, offset)
                updated.append(primary)
            for extra in existing[1:]:
                _CompositorInternal._delete_safely(extra)
