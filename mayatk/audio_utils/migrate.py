...
class _MigrateInternal(object):
    """Internal helpers for Migrate."""
        ...
        try:
            for attr in (trigger_attr, manifest_attr):
                if cmds.attributeQuery(attr, node=obj, exists=True):
                    ...
                    curves = (
                        cmds.listConnections(f"{obj}.{attr}", type="animCurve") or []
                    )
                    if curves:
                        cmds.delete(curves)
...
class Migrate(_MigrateInternal):
    ...
    def migrate_legacy_triggers(
        ...
        legacy_fm_attr = (
            "audio_file_map" if category == "audio" else f"{category}_file_map"
        )
        ...
        entries = _MigrateInternal._read_legacy_keys(obj, trigger_attr)
        if not entries:
            if not keep_old_attrs:
                _MigrateInternal._delete_legacy_attrs(
                    obj, trigger_attr, manifest_attr, legacy_fm_attr
                )
            return []
