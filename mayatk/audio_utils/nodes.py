...
class _NodesInternal(object):
    """Internal helpers for Nodes."""
...
class Nodes(_NodesInternal):
    ...
    def resolve_playable_path(
        ...
        if cache_dir is None:
            cache_dir = os.path.join(
                os.path.dirname(audio_path), _CACHE_DIR_NAME
            ).replace("\\", "/")
    ...
    def create_dg(
        ...
        if cmds is None:
            return None
        playable = Nodes.resolve_playable_path(file_path)
        if not playable:
            return None
        if name is None:
        ...
        Nodes.configure_dg(node, playable, offset)

        if track_id is not None:
            _NodesInternal._stamp_marker(node, track_id)
        return node
