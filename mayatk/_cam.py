...
#from this package:
from mayatk import _core, _node
...
class Cam(object):
	...
	@staticmethod
	@_core.Core.undo
	def groupCameras(name='cameras', non_default=True, root_only=False, hide_group=False):
		...
		default_cameras = ('side', 'front', 'top', 'persp', 'back', 'bottom', 'left', 'alignToPoly') # List of default cameras
		all_cameras = pm.ls(type='camera') # Get all cameras in the scene
		all_camera_transforms = [cam.getParent() for cam in all_cameras] # Get the parent transform nodes of the cameras

		if root_only: # Filter cameras based on the root_only flag
			all_camera_transforms = [cam for cam in all_camera_transforms if cam.getParent() is None]

		# Parent cameras to the group based on the non_default flag
		for cam in all_camera_transforms:
			if non_default:
				if not any([cam.name().endswith(def_cam) for def_cam in default_cameras]):
					pm.parent(cam, group)
			else:
				pm.parent(cam, group)
